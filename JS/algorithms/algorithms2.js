...
const rectangle = (base, height) => {
  console.log(
    `The area of the rectangle is ${(base * height).toFixed(
      2
...
const kilos2Pounds = (weight) => {
  console.log(
    `${weight} kilos are equivalent to ${(weight * 2.2).toFixed(2)} lbs`
  );
};
...
const triangle = (s1, s2, s3) => {
  if (s1 > 0 && s2 > 0 && s3 > 0) {
    ...
    const p = (s1 + s2 + s3) / 2;
    const area = Math.sqrt(p * (p - s1) * (p - s2) * (p - s3)).toFixed(2);
