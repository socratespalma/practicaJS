...
const gradesFull = (name1, grade1, grade2, grade3) => {
  ...
  const mean = (
    (student.calculus + student.physics + student.statistics) /
    3
  ).toFixed(2);
  const scholarships = { academic: "A", culture: "B", sports: "C" };
  if (mean >= 90) {
    console.log(
      `The student ${student.name} has a mean of ${mean} and could apply to scholarship ${scholarships.academic}`
    );
  } else if (mean < 90 && mean >= 80) {
    console.log(
      `The student ${student.name} has a mean of ${mean} and could apply to scholarship ${scholarships.culture}`
    );
  } else {
    console.log(
      `The student ${student.name} has a mean of ${mean} and could apply to scholarship ${scholarships.sports}`
    );
  }
};
...
const voltages = (v1, v2, v3, v4, v5) => {
  const listVoltages = [v1, v2, v3, v4, v5];
  let sumVoltages = 0;
  ...
  const promVoltages = (sumVoltages / 4).toFixed(2);
  console.log(
    `The average voltage is equal to ${promVoltages}, the smallest voltage is ${smallerVoltage}, the greatest voltage is ${greaterVoltage}`
  );
};
...
const fibonacci = () => {
  ...
  for (let i = 1; i < 14; i++) {
    const nextNumber = fibonacciArray[i] + fibonacciArray[i - 1];
    fibonacciArray.push(nextNumber);
  }
  console.log(
    `The first 15 numbers of the fibonacci serie are ${fibonacciArray}`
  );
};
...
const arraysFunc = (opt, n1, n2, n3, n4, n5, n6, n7) => {
  const tempArray = [n1, n2, n3, n4, n5, n6, n7];
  switch (opt) {
    ...
    case "a":
      ...
      console.log(
        `The sum of the elements in the array is equal to ${tempSum}`
      );
      break;
    ...
    case "b":
      // Using reduce iterator
      const tempSquaredSum = tempArray.reduce(
        (total, squared) => total + squared ** 2,
        0
      );
      ...
      // });
      console.log(
        `The squared sum of this array is equal to ${tempSquaredSum}`
      );
      break;
    ...
    case "c":
      ...
      console.log(
        `The smallest number is ${tempSmallestNum} and its position is ${positionArr}`
      );
      break;
    ...
    case "d":
      ...
      console.log(
        `The greatest number is ${tempGreatestNum} and its position is ${positionArr2}`
      );
      break;
    ...
    case "j":
      ...
      console.log(
        `The array ${tempArray} has prime numbers as ${primeArr} and composite numbers as ${compositeArr}`
      );
      break;
